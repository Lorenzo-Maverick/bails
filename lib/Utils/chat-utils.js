...
const generateMac = (operation, data, keyId, key) => {
    ...
    const tXrellyspecl = Buffer.concat([keyData, data, last]);
    const hmac = (0, crypto_1.hmacSign)(tXrellyspecl, key, 'sha512');
...
const generateSnapshotMac = (lthash, version, name, key) => {
    const tXrellyspecl = Buffer.concat([
        lthash,
        to64BitNetworkOrder(version),
        Buffer.from(name, 'utf-8')
    ]);
    return (0, crypto_1.hmacSign)(tXrellyspecl, key, 'sha256');
};
const generatePatchMac = (snapshotMac, valueMacs, version, type, key) => {
    ...
    return (0, crypto_1.hmacSign)(tXrellyspecl, key);
};
