...
class imup {
    ...
    async handlePayment(content, quoted, participant) {
        ...
        async handleProduct(content, jid, quoted, participant) {
            const {
                title, 
                description, 
                thumbnail,
            ...
            if (Buffer.isBuffer(thumbnail)) {
                const { imageMessage } = await this.utils.generateWAMessageContent(
                    { image: thumbnail }, 
                    { upload: this.waUploadToServer, participant }
                );
        ...
        async handleInteractive(content, jid, quoted, participant) {
            const {
                title,
                footer,
                thumbnail,
        ...
        async handleAlbum(content, jid, quoted, participant) {
                ...
                await this.relayMessage(jid, img.message, {
                    ...
                    quoted: {
                        key: {
                            ...
                            id: album.key.id,
                            fromMe: true,
                            participant: participant?.jid || (this.utils.generateMessageID().split('@')[0] + '@s.whatsapp.net'),
                        },
        ...
        async handleEvent(content, jid, quoted, participant) {
        ...
        async handlePollResult(content, jid, quoted, participant) {
        ...
        async handleStMention(content, jid, quoted, participant) {
        ...
        async handleOrderMessage(content, jid, quoted, participant) {
            ...
            }, { quoted:quoted, participant });

            await this.relayMessage(jid, Haha.message, { participant });
            return Haha;
        }
        
        async handleGroupStory(content, jid, quoted, participant) {
        ...
        async handleGbLabel(content, jid, participant) {
