...
const makeChatsSocket = (config) => {
    ...
    /** cache so we don't hit the server on every single message from the same LID */
    const lidToPnCache = new Map();
    /**
     * Resolve a `@lid` jid to the underlying `@s.whatsapp.net` (phone number) jid.
     * Returns undefined if WhatsApp won't tell us (e.g. the contact has PN-sharing off).
     */
    const getPNForLID = async (lidJid) => {
        var _a;
        const lidUser = (_a = (0, WABinary_1.jidDecode)(lidJid)) === null || _a === void 0 ? void 0 : _a.user;
        if (!lidUser) {
            return undefined;
        }
        ...
        try {
            const usyncQuery = new WAUSync_1.USyncQuery()
                .withContactProtocol()
                .withUser(new WAUSync_1.USyncUser().withLid(lidUser));
            const results = await sock.executeUSyncQuery(usyncQuery);
            const match = results === null || results === void 0 ? void 0 : results.list.find((a) => a.contact && (0, WABinary_1.isJidUser)(a.id));
            const pn = match ? (0, WABinary_1.jidNormalizedUser)(match.id) : undefined;
            lidToPnCache.set(lidUser, pn);
            return pn;
        }
        catch (error) {
            logger.debug({ lidJid, error }, 'failed to resolve PN for LID');
            return undefined;
        }
    };
