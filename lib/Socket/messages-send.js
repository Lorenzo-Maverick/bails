            ...
            if (messageType) {
                switch(messageType) {
                    ...
                    case 'PRODUCT':
                        ...
                        const productMsg = await Utils_1.generateWAMessageFromContent(jid, productContent, { quoted, participant: options.participant });
                    ...
                    case 'INTERACTIVE':
                        ...
                        const interactiveMsg = await Utils_1.generateWAMessageFromContent(jid, interactiveContent, { quoted, participant: options.participant });
                    ...
                    case 'ALBUM':
                        return await luki.handleAlbum(content, jid, quoted, options.participant)
                    case 'EVENT':
                        return await luki.handleEvent(content, jid, quoted, options.participant)
                    case 'POLL_RESULT':
                        return await luki.handlePollResult(content, jid, quoted, options.participant)
                    case 'STATUS_MENTION':
                        return await luki.handleStMention(content, jid, quoted, options.participant)
                    case 'ORDER':
                        return await luki.handleOrderMessage(content, jid, quoted, options.participant)
                    case 'GROUP_STATUS':
                        return await luki.handleGroupStory(content, jid, quoted, options.participant)
                    case 'GROUP_LABEL':
                        return await luki.handleGbLabel(content, jid, options.participant)
                }
            }
