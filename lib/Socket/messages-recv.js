                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if ((_b = msg.key.participant) === null || _b === void 0 ? void 0 : _b.endsWith('@lid')) {
                            const lidParticipant = msg.key.participant;
                            let resolvedParticipant = node.attrs.participant_pn;
                            if (!resolvedParticipant && getPNForLID) {
                                try {
                                    resolvedParticipant = await getPNForLID(lidParticipant);
                                }
                                catch (err) {
                                    logger.debug({ lidParticipant, err }, 'live participant LID->PN resolve failed');
                                }
                            }
                            // never default to our own id here - that would misattribute the message to ourselves
                            msg.key.participant = resolvedParticipant || lidParticipant;
                        }
                        ...
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            ...
                            let resolvedPn = node.attrs.sender_pn || node.attrs.peer_recipient_pn;
                            if (!resolvedPn && getPNForLID) {
                                try {
                                    resolvedPn = await getPNForLID(lidJid);
                                }
                                catch (err) {
                                    logger.debug({ lidJid, err }, 'live LID->PN resolve failed');
                                }
                            }
                            // never null out remoteJid - fall back to the original @lid if we still can't resolve it
                            msg.key.remoteJid = resolvedPn || lidJid;
                        }
